// src/pages/admin/MedecinsPage.tsx
...
export default function MedecinsPage() {
  ...
  useEffect(() => {
    specialitesApi.list()
      ...
      .catch(e => toast.error(e.message ?? 'Erreur chargement spécialités.'))
  ...
  const setFormField = <K extends keyof FormState>(k: K, v: FormState[K]) => {
    setForm(f => ({ ...f, [k]: v }))
    if (formErrors[k]) setFormErrors(e => ({ ...e, [k]: undefined }))
  }

  const handlePhotoChange = (file: File | null) => {
    setFormField('photo', file)
    if (photoPreviewUrl?.startsWith('blob:')) URL.revokeObjectURL(photoPreviewUrl)
    setPhotoPreviewUrl(file ? URL.createObjectURL(file) : (editing?.photo ?? undefined))
  }
  ...
  const openEdit = (m: Medecin) => {
    ...
    // m.photo est déjà une URL absolue grâce à MedecinResource
    setPhotoPreviewUrl(m.photo ?? undefined)
    setModal('edit')
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {loading ? (
        ...
        <>
          <Table headers={['Médecin', 'Spécialité', 'Contact', 'Disponibilité', 'Statut', 'Actions']} empty="Aucun médecin trouvé.">
            {medecins.map(m => (
              <Tr key={m.id}>
                <Td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    {/* ✅ m.photo = URL absolue (http://...) depuis MedecinResource */}
                    <Avatar src={m.photo ?? undefined} initials={m.initials} size={40} color={m.color} bg={m.bg_color} />
                    <div>
                ...
                <Td>
                  {m.specialite ? (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600, background: m.specialite.bg_color, color: m.specialite.color }}>
                ...
                <Td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: m.disponible ? T.green : T.gray300, flexShrink: 0 }} />
                    <span style={{ fontSize: 12, color: T.gray500 }}>{m.disponible ? 'Disponible' : 'Indisponible'}</span>
                  </div>
                </Td>
                ...
                <Td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <Btn variant="ghost" size="sm" onClick={() => handleToggle(m)} disabled={togglingId === m.id} icon={m.is_active ? <ToggleRight size={14} color={T.green} /> : <ToggleLeft size={14} />} />
      ...
      <Modal open={!!modal} onClose={closeModal} title={editing ? 'Modifier le médecin' : 'Nouveau médecin'} width={580}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: T.gray400, marginBottom: 12 }}>Informations générales</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <Input label="Nom complet" required value={form.name} onChange={v => setFormField('name', v)} placeholder="Dr. Prénom Nom" error={formErrors.name} />
                <Select label="Spécialité" required value={form.specialite_id} onChange={v => setFormField('specialite_id', v)} options={loadingSpecs ? [{ value: '', label: 'Chargement…' }] : specSelectOptions} error={formErrors.specialite_id} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <Input label="Téléphone" value={form.phone} onChange={v => setFormField('phone', v)} placeholder="07 XX XX XX XX" icon={<Phone size={14} />} />
                <Input label="Email" type="email" value={form.email} onChange={v => setFormField('email', v)} placeholder="medecin@esmad.ci" icon={<Mail size={14} />} />
              </div>
              <Input label="Biographie" value={form.bio} onChange={v => setFormField('bio', v)} rows={2} placeholder="Courte présentation du médecin…" />
            </div>
          </div>

          <div>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: T.gray400, marginBottom: 12 }}>Photo</div>
            {/* ✅ preview = blob URL si nouveau fichier, sinon URL absolue existante */}
            <FileInput label="Photo du médecin (optionnel)" onChange={handlePhotoChange} accept="image/*" hint="JPG, PNG, WEBP — max 2 Mo" preview={photoPreviewUrl} />
          </div>

          <div>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: T.gray400, marginBottom: 14 }}>Disponibilité & Statut</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Toggle checked={form.disponible} onChange={v => setFormField('disponible', v)} label="Médecin disponible (affiché comme disponible sur le site)" />
              <Toggle checked={form.is_active} onChange={v => setFormField('is_active', v)} label="Médecin actif (visible sur le site)" />
            </div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 8, borderTop: `1px solid ${T.border}` }}>
            <Btn variant="secondary" onClick={closeModal} disabled={saving}>Annuler</Btn>
            <Btn onClick={handleSave} disabled={saving}>{saving ? 'Enregistrement…' : editing ? 'Mettre à jour' : 'Créer le médecin'}</Btn>
          </div>
        </div>
      </Modal>
